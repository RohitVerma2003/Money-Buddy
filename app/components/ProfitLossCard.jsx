...
const ProfitLossCard = ({heading , income , expense , extra}) => {
  ...
  return (
    <View className={`w-full border-2 rounded-md p-2 mb-3 h-36 flex justify-around items-end overflow-hidden ${isDark ? 'bg-light-dark border-grey-white' : 'bg-light-green'}`}>
      <Text className='w-full font-doodle text-lg text-white'>
        {heading}
      </Text>
      <View className='w-full flex justify-between items-center flex-row'>
        <View className={`w-2/5 border-2 rounded-md bg-fade-green h-16 p-2 flex justify-between items-center flex-row ${isDark && 'border-grey-white'}`}>
        ...
        <View className={`w-2/5 border-2 rounded-md bg-red-400 h-16 p-2 flex justify-between items-center flex-row ${isDark && 'border-grey-white'}`}>
