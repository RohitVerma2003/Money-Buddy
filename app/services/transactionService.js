...
const useTransactionService = () => {
    ...
    const monthlyTransactionStats = async () => {
        try {
            ...
            const podq = query(podColRef, where('user_uid', '==', user.uid), where('date', '>=', twelveMonthsAgo), where('date', '<=', dateToday), orderBy('date', 'desc'))
    ...
    const yearlyTransactionStats = async () => {
        try {
            ...
            const podq = query(colRef, where('user_uid', '==', user.uid), orderBy('date', 'desc'))
            ...
            const firstPodTransaction = podQuerySnapshot.docs.reduce((earliest, doc) => {
                const transactionDate = doc.data().date.toDate()
                return transactionDate < earliest ? transactionDate : earliest
            }, new Date())

            const firstYear = firstTransaction.getFullYear() < firstPodTransaction.getFullYear() ? firstTransaction.getFullYear() : firstPodTransaction.getFullYear()
